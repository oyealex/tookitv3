...
class OperationLogQueryDTOTest {
    ...
    void testInvalidLimitMax() {
        ...
        assertFalse(violations.isEmpty());
        assertEquals(1, violations.size());
        assertEquals("{error.operationlog.query.limit.exceeded}", violations.iterator().next().getMessage());
    }

    @Test
    @DisplayName("验证失败 - keyword过长")
    void testInvalidKeyword() {
        ...
        assertFalse(violations.isEmpty());
        assertEquals(1, violations.size());
        assertEquals("{error.device.query.keyword.too.long}", violations.iterator().next().getMessage());
    }
